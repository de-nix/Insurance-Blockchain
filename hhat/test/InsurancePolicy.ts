...
describe("InsurancePolicy", function () {
    ...
    beforeEach(async function () {
        InsurancePolicyContract = await ethers.getContractFactory("InsurancePolicy");

        insurancePolicy = await InsurancePolicyContract.deploy();
        await insurancePolicy.deployed();

        const [owner2, insured2, insurer2, otherAccount2] = await ethers.getSigners();
        owner = owner2;
        insured = insured2;
        insurer = insurer2;
    ...
    describe("acceptPolicy", function () {
        it("Should accept the policy and transfer the premium to the insurer", async function () {
            ...
            const coverageAmount = ethers.utils.parseEther("0.5");
            const premium = ethers.utils.parseEther("0.01");
            const duration = 365;
            const insuredAddress = insured.address;


            const createPolicyTx = await insurancePolicy.connect(insurer).createPolicy(
                policyName,
                coverageAmount,
                premium,
                duration,30,
            ...
            expect(policyDetails[7]).to.equal(true);
        ...
        it("Should revert if called by a non-insured account", async function () {
            ...
            await createPolicyTx.wait();
            await expect(
                insurancePolicy.connect(insurer).acceptPolicy(0, {value: premium})
            ).to.be.revertedWith("Only the insured can accept the policy.");
        });
        ...
        it("Should revert if the incorrect premium amount is sent", async function () {
            ...
            await expect(
                insurancePolicy.connect(insured).acceptPolicy(0, {value: premium.mul(2)})
    ...
    describe("denyPolicy", function () {
        it("Should deny the policy and set isActive to false", async function () {
            ...
            await createPolicyTx.wait();
            await expect(
                insurancePolicy.connect(insured).denyPolicy(0)
            ).to.not.be.reverted;

            const policyDetails = await insurancePolicy.getPolicy(0);
            expect(policyDetails[7]).to.equal(false);
        });
        ...
        it("Should revert if the policy is already active", async function () {
            ...
            await expect(
                insurancePolicy.connect(insured).denyPolicy(0)
            ).to.be.revertedWith("Policy is already active.");
        });
        ...
        it("Should revert if called by a non-insured account", async function () {
            ...
            await createPolicyTx.wait();
            await expect(
                insurancePolicy.connect(otherAccount).denyPolicy(0)
            ).to.be.revertedWith("Only the insured can deny a policy.");
        });
    });
    ...
    describe("fileClaim", function () {
        it("Should file a claim successfully", async function () {
            ...
            await expect(
                insurancePolicy.connect(insured).fileClaim(0, description, amount)
            ).to.not.be.reverted;
    ...
    describe("acceptAndPayClaim", function () {
        ...
        it("Should revert if called by a non-insurer account", async function () {
            ...
            await tx3.wait()
            await expect(
                insurancePolicy.connect(otherAccount).acceptAndPayClaim(0, 0)
            ).to.be.revertedWith("Only the insurer can accept a claim.");
        });
        ...
        it("Should revert if the coverage amount is not enough to accept the claim", async function () {
            ...
            await tx2.wait()
            const description = "Test claim";
            const amount = ethers.utils.parseEther("0.7");
            await expect(
                insurancePolicy.connect(insured).fileClaim(0, description, amount)
            ).to.be.revertedWith("Claim exceeds coverage amount.");
        });
    });
    ...
    describe("denyClaim", function () {
        it("Should deny the claim", async function () {
            ...
            expect(claimDetails2[1]).to.equal(amount);
            expect(claimDetails2[2]).to.equal(false);
            expect(claimDetails2[3]).to.equal(false);
            await expect(
                insurancePolicy.connect(insurer).denyClaim(0, 0)
            ).to.not.be.reverted;

            const claimDetails = await insurancePolicy.getClaimDetails(0, 0);
            expect(claimDetails[1]).to.equal(amount);
            expect(claimDetails[2]).to.equal(false);
            expect(claimDetails[3]).to.equal(true);
        });
        ...
        it("Should revert if called by a non-insurer account", async function () {
            ...
            await expect(
                insurancePolicy.connect(otherAccount).denyClaim(0, 0)
            ).to.be.revertedWith("Only the insurer can deny a claim.");
        });
    });
    ...
    describe("payPremium", function () {
        it("Should pay the premium and update premiumDueDate", async function () {
            ...
            expect(updatedDueDate[6]).to.be.gt(initialDueDate[6]);
        });
        ...
        it("Should revert if the premium is not yet due", async function () {
            ...
            await expect(
                insurancePolicy.connect(insured).payPremium(0, {value: premium})
            ).to.be.revertedWith("Premium is not yet due.");
        });
        ...
        it("Should revert if the incorrect premium amount is sent", async function () {
            ...
            await expect(
                insurancePolicy.connect(insured).payPremium(0, {value: premium.mul(2)})
            ).to.be.revertedWith("Incorrect premium amount.");
        });
        ...
        it("Should revert if called by a non-insured account", async function () {
            ...
            await tx.wait();
            await expect(
                insurancePolicy.connect(otherAccount
                ).payPremium(0, {value: premium})
            ).to.be.revertedWith("Only the insured can pay the premium.");
        });
    });
    ...
    describe("getPolicy", function () {
        it("Should return the details of a policy", async function () {
            ...
            const policyDetails = await insurancePolicy.getPolicy(0);
            expect(policyDetails[0]).to.equal(policyName);
            expect(policyDetails[1]).to.equal(insurer.address);
            expect(policyDetails[2]).to.equal(insuredAddress);
            expect(policyDetails[3]).to.equal(coverageAmount);
            expect(policyDetails[4]).to.equal(premium);
            expect(policyDetails[5]).to.equal(duration);
            // expect(policyDetails[5]).to.equal(d);
            expect(policyDetails[7]).to.equal(false);
            expect(policyDetails[8]).to.equal(false);
        });
    });
    ...
    describe("getAllPoliciesForInsured", function () {
        it("Should return all policy IDs for a given insured address", async function () {
            ...
            const coverageAmount1 = ethers.utils.parseEther("1");
            const premium1 = ethers.utils.parseEther("0.1");
            const duration1 = 365;
            const insuredAddress1 = insured.address;
            const policyName2 = "Test Policy 2";
            const coverageAmount2 = ethers.utils.parseEther("2");
            const premium2 = ethers.utils.parseEther("0.2");
            const duration2 = 180;
            ...
            // Wait for the policy creation transaction to be mined and confirmed
            await createPolicyTx.wait();
            const createPolicyTx2 = await insurancePolicy.connect(insurer).createPolicy(
                policyName2,
                coverageAmount2,
                premium2,
                duration2,30,
            ...
            const policyIds = await insurancePolicy.getAllPoliciesForInsured(
                insuredAddress2
            );
    ...
    describe("getAllPoliciesForInsurer", function () {
        it("Should return all policy IDs for a given insurer address", async function () {
            ...
            const policyIds = await insurancePolicy.getAllPoliciesForInsurer(
                insurer.address
            );
            expect(policyIds.length).to.equal(2);
        });
    ...
    describe("getAllClaimsForInsurer", function () {
        it("Should return all claim IDs for a given insurer address", async function () {
            ...
            await insurancePolicy.connect(otherAccount).getAllClaimsForInsurer(
                insurer.address
            ).then(claimIds => expect(claimIds.length).to.equal(2));
    ...
    describe("createPolicy", function () {
        it("Should create a new policy and increase the policy count", async function () {
            ...
            expect(finalPolicyCount).to.equal(initialPolicyCount.add(1));
        });
    });
    ...
    describe("createPolicy1", function () {
        ...
        it("Should emit a PolicyCreated event", async function () {
            ...
            await expect(
                insurancePolicy.connect(insurer).createPolicy(
                    policyName,
                    coverageAmount,
                    premium,
                    duration,30,
